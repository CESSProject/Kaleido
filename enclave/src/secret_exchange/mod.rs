...
use super::serde_json::to_string;
...
pub extern "C" fn run_server(socket_fd: c_int, sign_type: sgx_quote_sign_type_t) -> sgx_status_t {
    ...
    let keys=ecckeys.get_ecc_keys();
    // Generate Keypair
    let ecc_handle = SgxEccHandle::new();
    let _result = ecc_handle.open();
    // let (prv_k, pub_k) = ecc_handle.create_key_pair().unwrap();

    let (attn_report, sig, cert) = match create_attestation_report(&keys.pkey, sign_type) {
        Ok(r) => r,
        Err(e) => {
    ...
    let (key_der, cert_der) = match cert::gen_ecc_cert(payload, &keys.skey, &keys.pkey, &ecc_handle) {
        Ok(r) => r,
        Err(e) => {
    ...
    let hello= "hello chiang";
    let sig_t=ecc_handle.ecdsa_sign_slice(hello.as_bytes(), &ecckeys.skey).unwrap();
    println!("This is x of sig(hello) :{:?}",sig_t.x);
    let sig_vec = [sig_t.x, sig_t.y].concat();
    let mut sig_out = [0u32; 16];
    sig_out.copy_from_slice(&sig_vec[..]);
    // debug!("signature: {:02x}", sig_out.iter().format(""));
    let sig = serde_json::to_string(&sig_out);
    let sig = match sig {
        Ok(data) => data,
        Err(_) => {
    ...
    println!("the sig msg is :{:?}",sig);
    let res=ecc_handle.ecdsa_verify_slice(hello.as_bytes(),&ecckeys.pkey,&sig_t).unwrap();
    println!("verify the result of signature :{}",res);
