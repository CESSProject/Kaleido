...
use crate::param::podr2_commit_data::PoDR2Data;
...
fn get_mht_root_sig(
    ...
    // (H(R))^sk
    Ok(cess_curve::sign_hash(&root_hash, &skey).base_vector().to_vec())
}

fn get_mht_leaves_hashes(
    data: &mut Vec<u8>,
    n_blocks: usize,
    ...
    for i in 0..n_blocks {
        ...
        if i == n_blocks - 1 {
            mi = data[i * block_size..].to_vec();
        } else {
            mi = data[i * block_size..(i + 1) * block_size].to_vec();
        }
...
fn gen_phi(
    ...
    for i in 0..n_blocks {
        ...
        // H(mi)
        let hash = hash(mi.as_slice());
        let bhash = hex_to_bigint(hash.base_vector());
